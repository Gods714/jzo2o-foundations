...
public class ServeItemServiceImpl extends ServiceImpl<ServeItemMapper, ServeItem> implements IServeItemService {
    ...
    public void deactivate(Long id) {
        ...
        LambdaQueryWrapper<Serve> queryWrapper = Wrappers.<Serve>lambdaQuery().eq(Serve::getServeItemId, id).eq(Serve::getSaleStatus, FoundationStatusEnum.ENABLE.getStatus());
        Integer count = serveMapper.selectCount(queryWrapper);
        if (count > 0){
            throw new ForbiddenOperationException("该服务项有区域在使用，请先删除关联的区域服务");
        }

...
public class RegionServiceImpl extends ServiceImpl<RegionMapper, Region> implements IRegionService {
    ...
    public void active(Long id) {
        ...
        LambdaQueryWrapper<Serve> queryWrapper = new LambdaQueryWrapper<Serve>().eq(Serve::getRegionId, id).eq(Serve::getSaleStatus, FoundationStatusEnum.ENABLE.getStatus());
        Integer count = serveMapper.selectCount(queryWrapper);
        if (count <= 0) {
            throw new ForbiddenOperationException("该区域下没有上架的服务，请先上架");
        }
    ...
    public void deactivate(Long id) {
        ...
        Integer count = serveMapper.selectCount(queryWrapper);
        if (count > 0){
            throw new ForbiddenOperationException("该区域下有上架的服务，请先下架");
        }
